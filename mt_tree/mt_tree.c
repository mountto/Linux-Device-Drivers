...
{
	MA_STATE(mas, mt, 0, 0);// Khai báo thao tác trên cây mt
	void *ptr;

	mas_lock(&mas); // nắm cái khóa lock
	mas_set(&mas, 3); //Set up một thao tác ở vị trí index 3
	ptr = mas_walk(&mas); // load nó
...
// khởi tạo maple tree rỗng với flag
