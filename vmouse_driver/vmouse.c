///
/// vmouse.c
/// https://github.com/yannik520/LVDD
...
static int __init vmouse_init(void)
{
	...
	vms->dev->keybit[BIT_WORD(BTN_MOUSE)] = BIT_MASK(BTN_LEFT) | BIT_MASK(BTN_RIGHT);
        ...
        error = input_register_device(vms->dev);
        error = sysfs_create_file(&vms->dev->dev.kobj, (const struct attribute *)&dev_attr_wheel_speed);
